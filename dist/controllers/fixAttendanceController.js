...
exports.getFixRequestById = exports.deleteFixRequest = exports.editFixRequest = exports.getFixRequestsByEmployee = exports.getAllFixRequests = exports.updateFixRequestStatus = exports.submitFixAttendanceRequest = void 0;
const notificationUtils_1 = require("../utils/notificationUtils");
const Prisma_1 = __importDefault(require("../utils/Prisma"));
// POST /attendance-fix
const submitFixAttendanceRequest = async (req, res) => {
    try {
        const { employeeId, date, requestType, requestedCheckIn, requestedCheckOut, reason } = req.body;
        const parsedDate = new Date(date);
        // Step 1: Check if employee was on leave on that day
        const leaveExists = await Prisma_1.default.leaveRequest.findFirst({
            where: {
                employeeId,
                status: 'Approved',
        ...
        const validCheckIn = requestType === 'CheckIn' || requestType === 'Both'
            ? requestedCheckIn && !isNaN(new Date(`${date}T${requestedCheckIn}`).getTime())
                ? new Date(`${date}T${requestedCheckIn}`)
                : null
            : null;
        const validCheckOut = requestType === 'CheckOut' || requestType === 'Both'
            ? requestedCheckOut
                ? (() => {
        ...
        const attendanceFix = await Prisma_1.default.attendanceFixRequest.create({
            data: {
                employee: {
        ...
        const emp = await Prisma_1.default.employee.findUnique({ where: { id: employeeId } });
...
exports.submitFixAttendanceRequest = submitFixAttendanceRequest;
const updateFixRequestStatus = async (req, res) => {
    ...
    try {
        ...
        if (status === "Approved") {
            if (!attendanceRecordId) {
                ...
                await Prisma_1.default.attendanceFixRequest.update({
                    where: { id: requestId },
                ...
                updatedOrCreatedRecord = await Prisma_1.default.attendanceRecord.update({
            ...
            if (parsedBreaks.length > 0 && attendanceRecordId) {
                await Prisma_1.default.break.deleteMany({ where: { attendanceRecordId } });
                await Prisma_1.default.break.createMany({
                    data: parsedBreaks.map((br) => ({
                        attendanceRecordId: attendanceRecordId, // ✅ ensured to be string
        ...
        const user = await Prisma_1.default.user.findUnique({ where: { id: request.employee.userId } });
        try {
            // 🔔 Prepare notification title and message
            const employeeName = `${user?.fullName}`;
            const approverName = await Prisma_1.default.user.findUnique({ where: { id: reviewerId } });
        ...
        const updatedRequest = await Prisma_1.default.attendanceFixRequest.update({
            where: { id: requestId },
            data: {
                status,
...
exports.updateFixRequestStatus = updateFixRequestStatus;
// GET /attendance-fix
const getAllFixRequests = async (req, res) => {
    const reviewerId = req.user?.userId;
    try {
        ...
        if (reviewer.role?.name === "admin") {
            const allRequests = await Prisma_1.default.attendanceFixRequest.findMany({
                include: {
                    employee: {
                        include: {
                            user: true,
        ...
        if (reviewer.subRole?.level !== undefined) {
            const requests = await Prisma_1.default.attendanceFixRequest.findMany({
                where: {
                    employee: {
                        user: {
                            subRole: {
...
exports.getAllFixRequests = getAllFixRequests;
const getFixRequestsByEmployee = async (req, res) => {
    const { employeeId } = req.params;
    if (!employeeId) {
        return res.status(400).json({ message: "Employee ID is required." });
    }
    try {
        const fixRequests = await Prisma_1.default.attendanceFixRequest.findMany({
            where: { employeeId },
            include: {
                reviewedBy: {
...
exports.getFixRequestsByEmployee = getFixRequestsByEmployee;
// update data 
const editFixRequest = async (req, res) => {
    const requestId = req.params.id;
    const { status, reason, requestType, requestedCheckIn, requestedCheckOut, remarks } = req.body;
    try {
        const existingRequest = await Prisma_1.default.attendanceFixRequest.findUnique({
            where: { id: requestId },
            include: {
                employee: {
                    include: {
        ...
        if (wasApproved && status !== 'Approved' && existingRequest.attendanceRecordId) {
            await Prisma_1.default.attendanceRecord.delete({
...
exports.editFixRequest = editFixRequest;
// delete entry
const deleteFixRequest = async (req, res) => {
    const requestId = req.params.id;
    try {
        const request = await Prisma_1.default.attendanceFixRequest.findUnique({
            where: { id: requestId },
        ...
        await Prisma_1.default.attendanceFixRequest.delete({
            where: { id: requestId },
        });
...
exports.deleteFixRequest = deleteFixRequest;
// get single fix attendance entry
const getFixRequestById = async (req, res) => {
...
exports.getFixRequestById = getFixRequestById;
